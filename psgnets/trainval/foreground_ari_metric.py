...
if __name__ == '__main__':
  ...
  twos = 2 * ones
  aris = adjusted_rand_index(
    # tf.one_hot(tf.reshape(ones, [4,-1]), depth=64),
    tf.one_hot(tf.random.uniform(shape=[4,60*80], minval=0, maxval=4, dtype=tf.int32), depth=64),
    tf.one_hot(tf.reshape(twos, [4,-1]), depth=64)
  )
