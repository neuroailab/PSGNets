...
def norm_activation(
  ...
  if layer == LAYER_BN_RELU:
    return batch_norm_relu(
        inputs, bn_train, relu=nonlinearity,
        init_zero=init_zero, data_format=data_format)
  elif layer in LAYER_EVONORMS:
    return evonorm(
        inputs, bn_train, layer=layer, nonlinearity=nonlinearity,
        init_zero=init_zero, data_format=data_format)
  elif layer == 'relu':
...
def residual_block(inputs, filters, train, strides,
  ...
  if pre_activation:
    inputs = norm_activation(inputs, train, data_format=data_format,
                             layer=norm_act_layer, freeze_bn=freeze_bn)
  if use_projection:
  ...
  if pre_activation:
    ...
    inputs = norm_activation(
        ...
        data_format=data_format, layer=norm_act_layer, freeze_bn=freeze_bn)
...
def resnet_generator(block_fn, layers, num_classes=None, get_features=False,
  ...
  def resnet_model(inputs, train=True, **kwargs):
    ...
    inputs = tf.layers.max_pooling2d(
        ...
        data_format=data_format)
    endpoints['block0_pool'] = tf.identity(inputs, 'initial_max_pool')
  ...
  resnet_model.default_image_size = 256
  return resnet_model
